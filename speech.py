...
class MyApplication:
    def __init__(self):
        ...
        if self.RVT_OPENAI_API_KEY is not None:
        ...
        if self.RVT_DEEPL_API_KEY is not None:
        ...
        # VBCABLE supplement
        self.output_device_supplement = tk.Label(self.audio_frame, text="if you want to talk friend, you should choice \n 'CABLE Input (VB-Audio Virtual C)' in Output Device.",  bg="#333333", fg="white")
        self.output_device_supplement.place(x=0, y=70)
    ...
    def load_audio_setting(self):
        try:
            ...
            print('set temp default device. Please set your devise in audio tab')
            devices = sounddevice.query_devices()
            default_device_info = sounddevice.default.device
            self.selected_input_device = devices[default_device_info[0]]['name']
            self.selected_output_device = devices[default_device_info[0]]['name']
    ...
    def record_audio(self, outputDirWav):
        ...
        try:
            ...
            if self.selected_input_device:
                ...
                print('input_device: ', self.selected_input_device)
            if self.selected_output_device:
                print('output_device: ', self.selected_output_device)
            ...
            with ThreadPoolExecutor(max_workers=5) as executor:
                ...
                while self.is_enabled:
                    if self.device_changed:
                        print("Device changed detected. Reinitializing...")
                        stream.stop_stream()
                        stream.close()
                        audio.terminate()
                        self.device_changed = False
                        self.record_audio(outputDirWav)
                        return
    ...
    def text_to_speech_google(self, text, language):
        ...
        try:
            ...
            if self.selected_output_device:
                ...
                print('output device is not found. please set in audio tab')
